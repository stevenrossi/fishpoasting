...
bookSource <- c("\nFrom The Fisheries and Fishery Industries of the United States, Published in 1884-1887 by the U.S. Government Printing Office")
...
df <- data.frame( desc=rep(NA,N), img=rep(NA,N) )
...
for( i in 1:N )
{
  ...
  df[i,"desc"] <- paste(desc,bookSource)
  df[i,"img"]  <- img
}

write.csv( df, row.names=FALSE, file="URLs.csv" )
